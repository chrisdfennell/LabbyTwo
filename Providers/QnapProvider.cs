...
public sealed class QnapProvider(IHttpClientFactory httpFactory, ILogger<QnapProvider> log) : IConnectionProvider
{
    ...
    private async Task<string> SidAsync(Connection connection, CancellationToken ct)
    {
    ...
    {
        var http = httpFactory.CreateClient(ProviderHttp.ClientName);
        var doc = XDocument.Parse(await http.GetStringAsync(BaseUrl(connection) + buildUrl(await SidAsync(connection, ct)), ct));
        ...
        {
            _sessions.TryRemove(connection.Id, out _);
            doc = XDocument.Parse(await http.GetStringAsync(BaseUrl(connection) + buildUrl(await SidAsync(connection, ct)), ct));
        }
        return doc;
    }
